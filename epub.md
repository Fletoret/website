...
| `sami-frasheri/shqiperia` | **Shipped** | Proofread against the 1899 Bucharest print (BKSH `libra1!HASH0181!8b1ffa07.dir`, slug `shqiperia`). The site's text came from a 2015 edition that had lost the end of *Besa e lidhja* and chapters V–XIII; they are restored from the scans, and the rest re-checked (titles to the print, with `slug:`). The print's subtitle is the book's `"subtitle"`. |
...
| `pashko-vasa/e-verteta-mbi-shqipnine` | **Shipped** | Mehdi Frashëri's 1935 translation (BKSH `libra1!HASH01fe!ff7391d4.dir`, pipeline slug `e-verteta-mbi-shqipnine`). The pipeline split at page turns, so each chapter's tail sat on the front of Frashëri's mid-page «Shënime» section; rebuilt from `pages/*.json` with the split points read off the scans, the notes kept as a `###` section of their chapter. Proofread against every scan (~155 fixes: words run together before `ësht`, a broken dotless `i` read as `j`, paragraphs wrongly split at page turns, printer's slips). Credits the translator via `"translatedBy"`. |
